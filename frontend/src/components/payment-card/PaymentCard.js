...
export default function PaymentCard() {
      ...
      .then((rest) => {
        ...
        if (status === 'requires_action') {
          stripe.confirmCardPayment(client_secret).then((result) => {
            if (result.error) showAlert('Payment Failed ! Please Try Again.', true);
            else {
              addSubscriptionDetialsToDatabase(state, result.paymentIntent.id, subscriptionId);
            }
            setLoading(false);
  ...
  return (
    <div className="payment__container">
      <div className="payment__info">
        <h2 className="payment__title">Complete Payment</h2>
        <small className="payment__description">Enter your credit or debit card details below</small>
        <form onSubmit={startSubscription} className="payment__form">
          <fieldset className="FormGroup">
            <div className="FormRow">
      ...
      <div className="order__info">
        ...
        <ul className="order__list">
          <li>
            Plan Name <span className="incapital">{state && state.plan}</span>
          </li>
          <hr />
          <li>
            Billing Cycle <span className="incapital">{state && state.period}</span>
          </li>
          <hr />
          <li>
            Plan Price{' '}
